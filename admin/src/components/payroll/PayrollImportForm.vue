<template>
  <v-card>
    <v-card-title>
      <span class="text-h6">Importar nueva archivo de liquidacion</span>
    </v-card-title>
    <v-card-text>
      <v-container>
        <v-form fast-fail ref="payrollTagForm">
          <VRow>
            <VCol cols="12">
              <v-btn
                block
...
const create = async () => {
  const { valid } = await payrollTagForm.value.validate()
  if (!valid) return
